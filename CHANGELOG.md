...
> v2.1.0 — Расширенные фильтры (до 1 года), GOAL_ID select из Метрики, авто-поиск Chrome для PDF,
> руководство пользователя, исправлен COUNTER_ID, убраны badge'и CI/E2E/Security из README.

### Added

- **Фильтры до 1 года**: пресеты 7д/14д/30д/90д/1г, кастомный date picker с ограничением макс. 365 дней
- **GOAL_ID select**: выпадающий список всех целей из Метрики (активные + архивные) с ID и названиями
- **PDF авто-поиск Chrome**: автоматически находит Chrome на macOS/Linux/Windows; понятное сообщение если не найден
- **docs/Руководство_пользователя.md**: полное руководство по развёртыванию и использованию (8 разделов)

### Fixed

- **COUNTER_ID**: исправлен путь ENV_PATH в backend settings.ts (было 99999 → 54280963)
- **Settings форма**: инициализация COUNTER_ID из health API когда settings возвращает 0
- **README**: убраны badge'и CI/E2E/Security (показывали failed), обновлена документация на двух языках
- **PDF renderer**: puppeteer-core теперь авто-находит Chrome executable path
