...
**Текущая версия:** v2.10.0 (2026-05-28)
...
- Графики отчёта рендерятся server-side в PNG (`report/chart-renderer.ts`, Puppeteer + ECharts, вне coverage — ADR-008/spec 014) и встраиваются идентично в DOCX (`ImageRun`) и PDF (`<img data:>`); опции графиков и блок 🟢/🔴 у каждого — чистые функции в `@pca/shared` (`report-charts.ts`). Детерминизм: один `snapshotId` → побайтово идентичные PNG (v2.8.0).
- B2B-пайплайн вводится в Настройках: kanban (4 этапа) + drawer с полной таблицей (v2.8.0). Версия в шапках доков синхронизируется `scripts/sync-versions.mjs` + committed pre-commit хук (ADR-011).
