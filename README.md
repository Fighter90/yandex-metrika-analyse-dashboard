...
> ✅ **Статус: рабочий продукт v2.5.4.** Дашборд из 9 страниц (Обзор, Трафик, Поведение,
...
> текущего COUNTER_ID, PDF авто-поиск Chrome, 397 тестов проходят, полный CI/CD.
> Проверено на живых данных счётчика ProductCamp (54280963).
...
6. **Отчёт.** `SnapshotBuilder` собирает **неизменяемый** `ReportSnapshot` из БД (детерминированно:
   ...
   puppeteer-core). **AI-анализ** генерится в 5 секций (chunked, по 45с каждая, с таймаутом),
...
| ------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Обзор**     | `/`         | KPI-цель, заявки, gap; графики визитов/заявок по дням; микс каналов; топ стран; доля устройств; UTM-разбивка; страницы входа/выхода; слабые места; цветовые бейджи |
...
| **Отчёт**     | `/report`   | Генерация снапшота; AI-анализ (5 секций, HTML-рендеринг); экспорт DOCX/PDF; перестроение                                                                           |
| **История**   | `/history`  | Список снапшотов; кнопка «Просмотреть» → открывает сохранённый отчёт                                                                                               |
| **Настройки** | `/settings` | OAuth-токен, Client ID/Secret, COUNTER_ID, GOAL_ID, ANTHROPIC_API_KEY; синхронизация с прогресс-баром (10 стадий); текущий счётчик                                 |
...
- Обложка (период, ID среза, цель), Краткие итоги (заявки B2C, оплачено B2B, gap — «заявка ≠ оплата»),
  Методология (Double Diamond + Воронкова, с атрибуцией);
...
- [x] Страница «Справка»: полная документация (12 разделов, 10 FAQ, глоссарий).
- [x] 397 тестов проходят (shared: 54, backend: 222, frontend: 121).
- [x] Проверено на живых данных Метрики (счётчик 54280963); security-аудит + `.gitignore`.
...
| -------------------------------------------------------------------------------------- | ---------- | --------------------------------------------------------------------------- |
| **v2.3.0** (Latest)                                                                    | 2026-05-27 | AI-анализ без обрезаний, md-to-html с таблицами/списками, DOCX/PDF GOST     |
