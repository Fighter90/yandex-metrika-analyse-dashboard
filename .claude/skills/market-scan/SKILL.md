<!--
Адаптировано из: Voronik1801 / Podlodka_crew_AI_Product
https://github.com/Voronik1801/Podlodka_crew_AI_Product (файл: skill-market-scan.md)
Методология НЕ изменена. Ниже — оригинал дословно; в конце добавлен доменный запрос ProductCamp.
-->

# Skill — Market Scan

> Превращает «погугли мой рынок» в структурированный запрос с источниками.

---

## Назначение

Получить картину рынка по теме за 15 минут вместо 4 часов. С источниками — иначе это галлюцинация.

## Инструмент

Любой из:
- EXA
- Perplexity (платно, лучше всего с источниками)
- Claude с web search (рядом по качеству)
- Tavily через API (open-source, для встраивания в скрипты)
- SearXNG + Llama локально (полный open-source, медленнее)

## Промпт-шаблон

```
Сделай market scan по теме: {точное описание ниши и сегмента}

Структура отчёта:

1. Размер рынка
   - Глобально (TAM)
   - В моих регионах: {регион}
   - Темп роста за последние 2 года
   - Источник для каждой цифры (без источника — не включай)

2. Игроки (минимум 5):
   - Название
   - Продукт (одной строкой)
   - ICP (для кого)
   - Ценник (от/до, модель)
   - Сильные стороны (2)
   - Слабые стороны (2)
   - Источник информации

3. Тренды (3):
   - Что меняется в отрасли
   - Кто двигает изменение
   - Сигнал для меня

4. Регуляторные риски:
   - Есть ли законы / стандарты, которые влияют
   - Сроки изменений

5. Белые пятна (2):
   - Что игроки НЕ закрывают
   - Где есть незанятая ниша

Правила:
- Каждая цифра — с источником. Без ссылки — не включай.
- Если данных нет — напиши «данных не нашёл», не выдумывай.
- Используй данные не старше 18 месяцев. Старее — помечай как историческое.
```

## После получения отчёта — проверочные вопросы

```
По этому отчёту:
1. Какие из утверждений я бы перепроверил вручную?
2. Какие источники могут быть предвзятыми (паблики компаний, маркетинговые материалы)?
3. Что в отчёте противоречит моему опыту? Почему?
4. Какие 3 вопроса я задам реальному человеку из этой индустрии?
```

## Метрика качества

Хороший market scan — это **отчёт, в котором есть минимум одна цифра, которая удивит**.
Если всё «как я и предполагал» — переформулируйте запрос на более узкий сегмент.

## Ограничения

- AI оптимизирует под «полный отчёт», а не точность — выдаёт цифры даже когда их нет
- Источники могут быть устаревшими — проверяйте дату
- Региональные данные (особенно RU/CIS) часто беднее западных — закладывайте время на ручной поиск

---

## Доменный запрос — ProductCamp «Конверсии и лидген»

Готовая подстановка в `{точное описание ниши и сегмента}`:

```
Российские продуктовые / IT-конференции и митапы 2024–2025 (офлайн + гибрид),
сегмент платных билетов B2C и корпоративных закупок B2B.
Фокус: ценообразование билетов (early bird / standard / B2B-пакеты),
каналы привлечения (подкасты, амбассадоры, инфопартнёры, рефералки),
способы оплаты (онлайн-карта vs счёт-реквизиты).
Регион: Россия / СНГ.
```

Что искать в первую очередь (сигналы для нашего KPI 300+ платных билетов):
- Как конкуренты дробят тарифы и когда поднимают цену (дедлайны early bird).
- Дают ли мгновенную онлайн-оплату — это прямой сигнал к solution-гипотезе про оплату.
- Какие каналы конкуренты называют главными — сверка с нашей разбивкой каналов.
