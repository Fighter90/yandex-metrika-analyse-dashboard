...
## 6. ⚠️ ПРОВЕРИТЬ ОТДЕЛЬНО (не покрыто автогейтом — ручной/внешний прогон)

> Эти три пункта **НЕ закрываются** автоматическими тестами/гейтом и требуют отдельной проверки
> человеком или внешней средой. Автогейт (раздел 0) их зелёным НЕ делает — отмечать вручную.

### 6.1. offline-docx — визуальная ГОСТ-проверка в Word/LibreOffice (нужен GUI)

- **Почему отдельно:** распаковка DOCX (`unzip` + `grep`/парс XML) подтверждает структуру (поля,
  отсутствие пустых прогонов, графики, отсутствие markdown/эмодзи/двойной нумерации), но **визуальную
  вёрстку проверить программно нельзя** — нужен графический офис.
- [ ] Открыть свежий DOCX **и** PDF в MS Word и LibreOffice Writer.
- [ ] Зафиксировать скриншоты в `qa/offline-audit-v2.9.1.md`: поля 30/15/20/20 мм, Times New Roman
      14pt, межстрочный 1.5, нумерация страниц внизу по центру, титул по ГОСТ, TOC с гиперссылками
      (клик → переход), 3 графика не обрезаны, AI-заголовки без эмодзи и двойной нумерации, нет 2+
      пустых абзацев подряд, текст по ширине.

### 6.2. ai-live-validation — живой прогон Anthropic (тратит токены)

- **Почему отдельно:** конвейер и санитайз покрыты юнит-тестами + структурной проверкой DOCX, но
  **реальная генерация делает платный вызов к Anthropic** — запускать только по согласованию.
- [ ] `POST /api/report/insights` (или кнопка «Сгенерировать AI-анализ» в `/report`) → дождаться 6
      секций: Итог / Каналы-UTM-Аудитория / Страницы-Воронка / Риски-Рекомендации /
      Гипотезы-Дорожная карта / Итоговый вывод.
- [ ] `GET /api/report/snapshot/:id` содержит `aiNarrative`.
- [ ] Экспорт DOCX с этим нарративом — без D-EMOJI / D-DOUBLE-NUM / D-VERBOSE (см. раздел 1).

### 6.3. a11y axe-core (автотест требует ADR на зависимость)

- **Почему отдельно:** `axe-playwright`/`@axe-core/playwright` НЕ в зависимостях; добавление пакета
  вне зафиксированного стека требует **ADR** (`docs/decisions/`). До ADR — проверка ручная.
- [ ] Ручной axe-прогон (DevTools axe / Lighthouse) по 9 страницам → 0 нарушений WCAG 2 A/AA.
- [ ] Мобильный скролл — покрыт автоматически (e2e-проект `mobile-iphone-14`, раздел 0).
- [ ] (Опц.) Согласовать ADR и добавить `e2e/a11y.spec.ts` с axe — тогда пункт уйдёт в автогейт.

---

## 7. Светофор готовности

- 🟢 **GO** — базовый прогон (раздел 0) зелёный; разделы 1–5 пройдены; **раздел 6 подтверждён
  отдельно** (offline-docx визуально, ai-live при согласии, a11y вручную или по ADR); 0 Blocker/Critical.
- 🟡 **CONDITIONAL** — есть Major, базовые сценарии и отчёты работают; раздел 6 не весь закрыт.
- 🔴 **NO-GO** — есть Blocker/Critical или красный автогейт.

> Раздел 6 НЕ входит в автогейт: автотесты/CI его зелёным не делают. Пока он не подтверждён вручную —
> релиз считается «GO без AI» (как v2.9.0), а не «GO с AI без оговорок».
