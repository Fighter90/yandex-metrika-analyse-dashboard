...
| **Стадия**       | v2.7.1 — рабочий продукт, фазы A–D завершены                                                                                                                                                                                   |
...
| `hypotheses`       | гипотезы в формате методологии      | `ice_score` GENERATED `impact*confidence*ease`; FK `parent_id` → problem |
...
- Гипотеза не сохраняется без полного формата методологии (≥3 допущения по 3 категориям, ≥2 метода) —
...
Навигация (`Layout.tsx`) — 9 страниц, каждый график с подписью «🟢 корректно / 🔴 внимание / 💡 рекомендация».
Страницы «Гипотезы», «Решения» и «B2B» удалены из навигации в v2.7.0: гипотезы и Decision Log
генерируются AI и встраиваются в отчёт; ввод B2B-сделок перенесён в секцию «B2B-пайплайн» Настроек.
URL `/hypotheses`, `/decisions` → `/report`; `/b2b` → `/settings`.

| Страница      | Что показывает                                                                                                                                                                   |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Обзор**     | дайджест за неделю (WoW), KPI-стрип (метка «Оплат» для целей-покупок), графики: визиты/заявки по дням, микс каналов, **топ стран** (bar), **доля устройств** (donut), weak spots |
| **Трафик**    | бар каналов, grouped-bar «визиты vs заявки», UTM-Sankey (источник→кампания→заявки), таблица каналов, UTM-разбивка, бейдж низкого покрытия UTM                                    |
| **Поведение** | страницы входа (startURL) и выхода: графики CR/отказов + таблицы (выход пуст — Метрика не отдаёт `ym:s:exitURL`, см. cheatsheet)                                                 |
| **Воронка**   | воронка «заявка ≠ оплата» (Визиты → Заявки B2C → Билеты B2B → Оплачено B2B), CR по каналам, воронка по каналам, конверсия по странам                                             |
| **Цели**      | список целей Метрики, авто-KPI, what-if симулятор (доп. трафик % / рост CR %)                                                                                                    |
| **Отчёт**     | сборка snapshot; **AI-гипотезы** (problem/solution, из `snapshot.generatedHypotheses`) и **AI Decision Log** (из `snapshot.generatedDecisions`) внутри отчёта; экспорт DOCX/PDF  |
| **История**   | список сформированных snapshot-ов с переходом на просмотр                                                                                                                        |
| **Настройки** | ключ/токен/счётчик (save/clear), кнопка «Обновить данные» с прогрессом, сворачиваемая секция **«B2B-пайплайн»**                                                                  |
| **Справка**   | методология (DD/ICE), принципы, глоссарий                                                                                                                                        |
...
| GET   | `/api/metrics/raw/:id`      | сырой ответ по ID (прослеживаемость цифр)    |
...
7. **Текущий релиз:** v2.7.1. Семантическое версионирование + CHANGELOG.
